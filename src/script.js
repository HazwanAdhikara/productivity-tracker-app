...
function parseTime(timeStr) {
  ...
  return (
    parseInt(minutes) * 6000 + parseInt(seconds) * 100 + parseInt(hundredths)
  );
}
...
function startTimer(index) {
  if (!intervals[index]) {
    intervals[index] = setInterval(() => {
      timers[index]++;
      // Update the corresponding timer display
      document.querySelectorAll(".col .text-center p:nth-child(2)")[
        index
      ].textContent = formatTime(timers[index]);
...
function loadFromLocalStorage() {
  const pastActivities =
    JSON.parse(localStorage.getItem("pastActivities")) || [];
...
function stopTimer(index) {
  if (intervals[index]) {
    ...
    const activityName = document.querySelectorAll(".col .text-center h3")[
      index
    ].textContent;
    const elapsedTimeStr = document.querySelectorAll(
      ".col .text-center p:nth-child(2)"
    )[index].textContent;
